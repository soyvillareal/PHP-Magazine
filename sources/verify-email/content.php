...
$page = Specific::ValidateToken($verify_email['expires'], 'verify_email') || $verify_email['count'] == 0 || $dba->query('SELECT status FROM '.T_USER.' WHERE id = ?', $verify_email['user_id'])->fetchArray(true) == 'active' || (strlen($TEMP['#descode']) != 6 && !empty($TEMP['#descode'])) ? 'invalid-auth' : 'check-code';


$TEMP['title'] = $TEMP['#word']['check_your_email'];
$TEMP['type'] = 'verify_email';
$TEMP['token'] = $tokenu;
$TEMP['url'] = Specific::Url($RUTE['#r_verify_email']);
