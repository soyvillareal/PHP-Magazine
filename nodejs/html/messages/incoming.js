...
module.exports = function(socket, temp){
    return (temp.deleted_fprofile == 0 ?
        (temp.type == 'answered' ? 
            `<div class="content_pmtmessage display-flex direction-colum position-relative margin-l10" data-id="{!id}">
                <div class="item_pmtmessage position-relative padding-l20 margin-l15" data-id="{!answered_id}" data-type="{!ans_type}">
                    <h4 class="font-low font-normal color-grey">
                    ...
                    <div class="container-pmtans display-flex direction-row align-center">`+(
                        temp.ans_deleted == !1 ?
                            (temp.ans_type == 'text' ?
                                `<p class="item_pmtmmessage break-all background-grely padding-t5 padding-b15 padding-l15 padding-r15" data-id="{!ans_id}" data-type="text">{!ans_text}</p>`
                            :
                                (temp.ans_type == 'file' ?
                                    `<div class="content-pmtfile content_pmtfile display-flex direction-row align-center background-grely position-relative margin-t5 border-r25px" data-id="{!fi_aid}" data-type="file">
                ...
                <div class="item-pmtreply display-flex direction-row align-center position-relative margin-t15">
                    <div class="item-mavatar margin-top-auto margin-r5 overflow-hidden border-rall">
                        ...
                        temp.text != null ?
                            ...
                            <button class="btn_pmtreply w-30px h-30 btn-noway margin-l5 margin-r5 padding-5 background-hover color-grey border-rall opacity-0 hover-button animation-ease3s animate-tab-button" data-id="{!id}" data-type="text">
                                <svg class="icon-x vertical-middle" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    <path fill="currentColor" d="M0.389 8.899L8.64 1.774C9.362 1.151 10.5 1.657 10.5 2.626v3.752c7.529 0.086 13.5 1.595 13.5 8.731 0 2.88 -1.855 5.733 -3.906 7.225 -0.64 0.466 -1.552 -0.119 -1.316 -0.873 2.126 -6.797 -1.008 -8.602 -8.278 -8.707V16.875c0 0.97 -1.139 1.474 -1.86 0.851l-8.25 -7.125c-0.519 -0.448 -0.52 -1.254 0 -1.703z"/>
                                </svg>
                            </button>
                            <button class="btn_pmtmdelete w-30px h-30 btn-noway margin-r5 background-hover border-rall opacity-0 hover-button animation-ease3s animate-tab-button" data-id="{!id}" data-type="text">
    ...
    :
        ...
        temp.text != null ?
            (temp.messafi == !0 ?
                ((temp.has_file == !0 ?
                    `<div class="content_pmtfiles display-grid margin-l10">
                        {!infiles}
                    </div>`
                :
                    '')+
                (temp.has_image == !0 ?
                    `<div class="content_pmtimages display-grid margin-l10">
                        {!inimages}
                    </div>`
                :
                    ''))
        ...
        :
            ''
    )+`<div class="item_pmttime margin-t5 margin-l15 font-lowly color-grey">{!created_at}</div>`;
};