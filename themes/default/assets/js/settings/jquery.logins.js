...
(function(){
    ...
    $(document).on('click', '.btn_dalert', function(){
        ...
        var r = $(this),
            v = r.attr('data-id');
        v && (u(r.addClass('spinner-is-loading'), !0), $.post(paper_sl.url.delete_session+"?token="+paper_sl.token, {user_id: paper_sl.user_id, session_id: v}, function(e){
            if(e.S == 200){
                if(e.UR != undefined){
                    window.location = e.UR
                } else {
                    $('.content-row-table[data-id='+v+']').remove(), y(r), x()
                }
            } else {
                y(r), x()
            }
        ...
        n.removeClass('hidden'), t && (t = !1, $.post(paper_sl.url.table_sessions+"?token="+paper_sl.token, {user_id: paper_sl.user_id, page_id: s}, function(e){
            if(e.S == 200) {
