(function(){
	var g = function(e = '', i = !1, u = ''){
        ...
        clearTimeout(t), u == 'blue' ? x.addClass('background-blue').removeClass('background-red') : (u == 'red' ? x.addClass('background-red').removeClass('background-blue') : x.removeClass('background-blue background-red')), x.css('left', (i ? 20 : '')).find('p').text(e), t = setTimeout(function(){
            g()
        }, 5000);
    },
    t = 0;
   	...
   	(function(){
		...
		paper_p.nodejs && paper_p.socket.on('setOutcreaction', function(e){
			if(e.S == 200){
         	...
         	q && (i(v, !0), $.post(paper_p.url.comment+"?token="+paper_p.token, {post_id: x, text: y.val()}, function(e){
	            if(e.S == 200){
         	...
         	var v = $(this),
             	s = h(v);
         	q && (i(v, !0), $.post(paper_p.url.reply+"?token="+paper_p.token, {comment_id: v.attr('data-id'), text: s.val()}, function(e){
	            if(e.S == 200){
			...
			if(q){
				...
				if(!paper_p.nodejs){
					...
					paper_p.socket.emit('setIncrection', w, z)
				}
			}
			...
			q && (i(v, !0), $.post(paper_p.url.pin+"?token="+paper_p.token, {comment_id: v.attr('data-id'), sort_by: m(v)}, function(e){
         	...
         	var v = $(this),
				...
				w = {comment_id: v.attr('data-id'), type: s};

			if(q){
				i(v, !0);
				if(!paper_p.nodejs){
					$.post(paper_p.url.delete_comment+"?token="+paper_p.token, w, z)
				} else {
					paper_p.socket.emit('setIncdelete', w, z)
				}
			}
      	}), $('#btn-ccancel').click(l);
   })(), (function(){
