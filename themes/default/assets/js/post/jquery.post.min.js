      ...
      (function(){
         ...
         JSON.parse(paper_p.loggedin) && ($(document).on('click', '.btn_save', function(){
            var t = $(this),
                ...
                c,
                y;
            d.index(h[0]) == -1 && d.push(h[0]), d.index(t) == -1 && d.push(t), c = t.siblings('.content_saved'), y = d.find('path'), r(d, !0), $.post(paper_p.url.save+"?token="+paper_p.token, {'post_id': u}, function(e){
            ...
            i(v, !0), $.post(paper_p.url.delete+"?token="+paper_p.token, {'post_id': v.attr('data-id')}, function(e) {
               ...
               } else (r(v.removeClass('spinner-is-loading'), !1), console.log(e.E));
            });
         }));
      })(), (function(){
         var x = $('#posts'),
         ...
         $(document).scroll(function(){
            ...
            if(w < v){
               ...
               if(((w + $(window).height()) - x.offset().top) >= (x.height() - 100)){
                  if(n[u] == n[n.length-1]){
                     k && (k = !1, f.removeClass('hidden'), $.post(paper_p.url.load+"?token="+paper_p.token, {'category_id': paper_p.category_id, 'post_ids': JSON.stringify(n)}, function(e) {e.S == 200 ? (x.append(e.HT), n.push(e.ID), k = !0, f.addClass('hidden')) : f.addClass('hidden')}));
                  }
               }
         ...
         $(document).on('click', '.btn_plike, .btn_pdislike', function(){
            var x = $(this),
                y = x.parents('.container_likes');

            i(x, !0), $.post(paper_p.url.post_reaction+"?token="+paper_p.token, {post_id: x.attr('data-id'), type: x.attr('data-type')}, function(e){
               console.log(e)
               if(e.S == 200){
                  e.DO && k(y.find(e.DO)).find('.item_likedis').text(e.CO), x.find('.item_likedis').text(e.CR), e.DB && k(y.find(e.DB)), e.AB && y.find(e.AB).removeClass('color-grey').addClass('color-blue'), r(x.removeClass('spinner-is-loading'), !1);
               } else {
            ...
            d && (o = {reported_id: v, type: w, place: p, description: d.val()}), a.text(''), r(s, !0), i(x, !0), $.post(paper_p.url.report+"?token="+paper_p.token, o, function(e){
               console.log(e);
               if(e.S == 200){
            ...
            $('.item_rinput:checked') && r(t, !1), a.text(''), j.val('').next().find('.text_email_count').text(0), h()
         }), j.on('keyup, input',function(){
            var x = $(this),
                y = x.parent(),
                u = y.find('.text_email_count'),
                o = u.parent(),
                d = x.val(),
                v = d.length;
            (v > 400) ? (o.addClass('color-red').removeClass('color-green color-orange')) : ((v > 250) ? (o.addClass('color-orange').removeClass('color-red color-green')) : (o.addClass('color-green').removeClass('color-red color-orange'))), u.text(v), x.val(d.slice(0, 499))
         }), $(window).resize(h)
      })()
   })()
})(paper_p);