      ...
      (function(){
         ...
         JSON.parse(paper_p.loggedin) && ($(document).on('click', '.btn_save', function(){
            var t = $(this),
                ...
                c,
                y;
            d.index(h[0]) == -1 && d.push(h[0]), d.index(t) == -1 && d.push(t), c = t.siblings('.content_saved'), y = d.find('path'), r(d, !0), $.post(paper_p.url.save+"?token="+paper_p.token, {'post_id': u, 'post_ids': JSON.stringify(n)}, function(e){
            ...
            var v = $(this);
            v.addClass('spinner-is-loading'), r(v, !0), $.post(paper_p.url.delete+"?token="+paper_p.token, {'post_id': v.attr('data-id')}, function(e) {
               if(e.S == 200) {
            ...
            v.addClass('spinner-is-loading'), r(v, !0), $.post(paper_p.url.subscribe+"?token="+paper_p.token, {'email': k.val(), 'type': q.find('.newsletter_type').val(), 'frequency': q.find('.newsletter_frequency').val(), 'popular': q.find('.newsletter_popular:checked').val(), 'category': JSON.stringify(q.find('.newsletter_category').val())}, function(e) {
