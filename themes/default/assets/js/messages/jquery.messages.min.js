(function(){
	...
	(function(){
		...
		u.scrollTop(c.height()).removeClass('pointer-events-none'), q = !0, (function(){
			...
			(function(){
				var i = $('#content-mfrefi'),
					...
					q = function(e){
						...
						t.append('count_files', g.length), t.append('profile_id', paper_m.profile_id), t.append('text', n), t.append('answered_id', y), t.append('type', l), j = !1, w(s, !0), p.addClass('spinner-is-loading'), $.ajax({
				...
				(function(){
					...
					paper_m.enable_msg ? (p.click(q), b.focus(function(){
						...
						clearTimeout(h), v.length > 0 ? (v != j && (m = !0), j = v, h = setTimeout(function(){
				      			m = !1;
				      		}, 5000)) : (m = !1), (e.keyCode || e.which) == 13 && !e.shiftKey && v.length > 0 && q()
					...
					$(document).on('click', '.btn_pmtreply', function(){
						var v = $(this),
							...
							s = x.find('img').attr('alt'),
							k = g.parent(),
							c = x.find('.item_pmtext').text();

						if(['file', 'image'].indexOf(h) != -1){
							x = v.parents('.content_pmt'+h), o = x.find('.item_pmtufile'), o.length > 0 && (s = o.text()), c = h == 'file' ? paper_m.word.attached_file : paper_m.word.image
						}

						if(s != undefined){
							k.removeClass('hidden'), g.text(s), m.addClass('hidden')
						} else {
							...
							l(v), $.post(paper_m.url.delete_both_message+"?token="+paper_m.token, {id: s, for: u, type: x}, function(e){
								if(e.S == 200){
									...
									if(u == 'all'){
										...
										y.remove(), c.find('.item_pmtmessage[data-id='+s+'][data-type='+x+']').remove()
									}
								} else n(v)
							})
						}), g.click(function(){
				...
				var j = $('#item-mffile'),
					        ...
					        var p = new FileReader(),
					        	v = function(u){
					        		var l = $($('.content_mfimage')[0]),
										y = $('.item_mffile'),
										x = l;
					        		i && (x = l.clone(), l.val('')), x.removeClass('hidden').find('img').attr('src', u).attr('alt', u[0].name), y.length > 0 ? x.insertBefore(a.children().first()) : i && a.prepend(x), w(j, !1).parent().removeClass('spinner-is-loading'), s('image'), e(), b.focus()
					        	};
					...
					s = function(e){
						c.prepend(j.clone().attr('class', 'item_mffile is_'+e+' hidden').removeAttr('id')), a.removeClass('hidden'), g.removeClass('hidden'), j.val(''), u.css('height', 'calc(70vh - '+d.height()+'px)'), b.focus()
					};
				...
				j.change(function(e){
					...
					if(['image/png', 'image/jpeg', 'image/gif'].indexOf(k[0].type) != -1){
						w(v, !0).parent().addClass('spinner-is-loading');
				...
				}), $(document).on('click', '.btn_mffile', function(){
					var v = $(this),
			...
			u.scroll(function(){
				...
				j && (x.length > 0 && (l = {profile_id: paper_m.profile_id, messages_ids: JSON.stringify(x), type: 'last', keyword: y, typing: m}), k.length > 0 && (l = {profile_id: paper_m.profile_id, last_cupdate: paper_m.last_cupdate, profiles_ids: JSON.stringify(k), messages_ids: JSON.stringify(x), type: 'last', keyword: y, typing: m}), $.post(paper_m.url.fetch+"?token="+paper_m.token, l, function(e){
			        if(e.S == 200) {
			            !e.HTM && e.HTT ? (d.length == 0 && c.append(e.HTT), u.scrollTop(c.height())) : (d.remove(), m = !1), e.UIDS && (paper_m.profiles_ids = e.UIDS, paper_m.last_cupdate = e.LCU), e.HTM && c.find('.content_nomgs').length > 0 && c.html(''), e.HTU && g.length > 0 && g.remove(), !t.hasClass('is_new') && s.after(e.HTU), e.MIDS && (paper_m.messages_ids = e.MIDS, h()), f(e, !1), e.US && $.each(e.US, function(i, u){
			...
			}, 2000), d.on('keyup, keydown, input', function(){
		      	var v = d.val();
		...
		var m = $('#container-messages'),
			...
			f = function(){
				q.toggleClass('hide-mobile').next().toggleClass('hide-mobile')
			},
			y = function(e){
